...
const apiUrl = 'https://script.google.com/macros/s/AKfycbxhIy3tfOItHh6ChO1XSXBeYO4d8dBKyuOGD-pPgrGKzlXhGLbKrARWtiba1DOUt22G/exec';
...
const tableHeaders = ref([
  ...
  { key: 'lampiranDokumentasi', label: 'Dokumentasi', sortable: false, isRating: false },
  { key: 'pemateriMenjelaskanMateriDenganBaikDanDapatDipahami', label: 'Eval: Pemateri', sortable: true, isRating: true },
  { key: 'materiTersusunDenganBaikDanMudahDipahami', label: 'Eval: Materi', sortable: true, isRating: true },
  { key: 'penggunaanWaktuTrainingEfektifDanEfisien', label: 'Eval: Waktu', sortable: true, isRating: true },
  { key: 'fasilitasYangDigunakanDalamTrainingMemadai', label: 'Eval: Fasilitas', sortable: true, isRating: true },
  { key: 'perlengkapanYangDipergunakanDalamTrainingLengkap', label: 'Eval: Perlengkapan', sortable: true, isRating: true },
  { key: 'trainingIniSangatBermanfaatBagiSaya', label: 'Eval: Manfaat', sortable: true, isRating: true },
  { key: 'konsumsiPelatihanSesuaiHarapan', label: 'Eval: Konsumsi', sortable: true, isRating: true },
  { key: 'timestamp', label: 'Waktu Lapor', sortable: true, isRating: false},
]);
...
const fetchData = async (force = false) => {
  ...
  error.value = null;
  try {
    const response = await axios.get(`${apiUrl}?sheet=evaluasi`);
      ...
      <table v-if="paginatedData.length > 0">
        ...
        <tbody>
          <tr v-for="row in paginatedData" :key="row.uid">
            ...
            <ProgressBar v-if="header.isRating" :rating="Number(row[header.key])" />
                        <a v-else-if="String(row[header.key]).startsWith('http')" :href="row[header.key]" target="_blank" rel="noopener noreferrer">
                            Lihat File
                        </a>
